...
#CBAM改
class SPSCAM(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1,  ratio=16, kernel_size=7):  # ch_in,  number, shortcut, groups, expansion
        super(SPSCAM, self).__init__()
        self.channel_attention = ChannelAttention(c1, ratio)
        self.spatial_attention = SpatialAttention(kernel_size)

    def forward(self, x):
        out1 = self.channel_attention(x)
        out2 = self.spatial_attention(x)
        out11 = out1 * out2 * x
        out3 = self.spatial_attention(out11)
        out4 =self.channel_attention(out11)
        out21 = out3 * out4 * out11
        return out21
...
class CBAMBottleneck1(nn.Module):
    ...
    def forward(self, x):
        x1 = self.cv2(self.cv1(x))
        out1 = self.channel_attention(x1)
        out2 = self.spatial_attention(x1)
        out11 = out1 * out2 * x1
        out3 = self.spatial_attention(out11)
        out4 = self.channel_attention(out11)
        out21 = out3 * out4 * out11
        return out21 + x if self.add else out21
...
class CBAMBottleneck2(nn.Module):
    ...
    def forward(self, x):
        ...
        out2 = self.spatial_attention(x1)
        out11 = out1 * out2 * x1
        return x + out11 if self.add else out11


#C3CBAM改
class C32(C3):
    # C3 module with CBAMBottleneck1()
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(CBAMBottleneck1(c_, c_, shortcut) for _ in range(n)))
...
class C33(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(CBAMBottleneck2(c_, c_, shortcut) for _ in range(n)))


class SPDCM(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(SPDCM, self).__init__()
        self.aspp1 = Aspp_module(in_channels, out_channels, 5)
        self.aspp2 = Aspp_module(in_channels, out_channels, 3)
        self.aspp3 = Aspp_module(in_channels, out_channels, 7)
    def forward(self, x):
        x1 = self.aspp1(x)
        x2 = self.aspp2(x)
        x3 = self.aspp3(x)
...
class Aspp_module(nn.Module):
    def __init__(self, in_channels, out_channels, rates):
        ...
        if rates == 1:
            ...
            kernel_size = 3
            padding = rates
        self.atrous_conv = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=padding, dilation =rates, bias=False)
